GUNYAHState *get_gunyah_state(void) { return GUNYAH_STATE(current_accel()); }
...
static int gunyah_init_vcpu(CPUState *cpu, Error **errp) {
  ...
  cpu->accel->fd = -1;
  cpu->accel->run_size = qemu_real_host_page_size();
...
static void gunyah_vcpu_destroy(CPUState *cpu) {
  int ret;
  ret = munmap(cpu->accel->run, cpu->accel->run_size);
  if (ret < 0) {
...
static int gunyah_handle_unknown_exit(CPUState *cpu, struct gh_vcpu_run *run) {
  ...
  if (!gunyah_addr_is_lend(unk_addr) && run->mmio.len > 0 &&
      run->mmio.len <= 8) {
    bql_lock();
    address_space_rw(&address_space_memory, unk_addr, MEMTXATTRS_UNSPECIFIED,
...
static int gunyah_vcpu_exec(CPUState *cpu) {
  ...
  do {
    ...
    if (ret < 0) {
      ...
      if (errno == ENODEV) {
        if (qatomic_read(&gunyah_vm_stopped) ||
            qatomic_read(&cpu->exit_request) || cpu->cpu_index == 0) {
          ret = EXCP_INTERRUPT;
          break;
        }
        usleep(10000);
