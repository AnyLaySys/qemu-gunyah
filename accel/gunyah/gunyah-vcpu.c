...
static int gunyah_init_vcpu(CPUState *cpu, Error **errp) {
    ...
    gh_report("init_vcpu %d (deferred - VCPU will be created at VM start)", cpu->cpu_index);
    return 0;
}
...
void *gunyah_cpu_thread_fn(void *arg) {
    ...
    current_cpu = cpu;
    gh_report("cpu_thread_fn started for cpu %d (tid=%d)", cpu->cpu_index, cpu->thread_id);
    gunyah_init_vcpu(cpu, &error_fatal);
    cpu_thread_signal_created(cpu);
    qemu_guest_random_seed_thread_part2(cpu->random_seed);
    gh_report("cpu %d entering main loop (fd=%d run=%p)", cpu->cpu_index, cpu->accel->fd,
                 cpu->accel->run);
    do {
        if (cpu_can_run(cpu)) {
