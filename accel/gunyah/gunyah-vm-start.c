...
static void gunyah_dump_vm_state(GUNYAHState *s) {
    int i;
    error_report("fd=%d vmfd=%d protected=%d swiotlb_size=0x%"
    PRIx64, s->fd, s->vmfd, s->protected_vm, s->swiotlb_size);
    error_report("nr_slots=%u nr_irqs=%u preshmem_reserved=%d", s->nr_slots, s->nr_irqs,
                 s->preshmem_reserved);
...
void gunyah_start_vm(void) {
    ...
    CPU_FOREACH(cpu)
    {
        ...
        if (ret < 0) {
            error_report("could not create VCPU %d: %s (errno=%d)", vcpu_arg.id, strerror(errno),
                         errno);
            exit(1);
        }
...
#define PCI_NUM_SPIS  4
        ...
        for (i = 0; i < nbell; ++i) {
            ...
            if (bells[i].label >= PCI_FIRST_SPI && bells[i].label < PCI_FIRST_SPI + PCI_NUM_SPIS) {
    ...
    if (s->dtb_start) {
        ...
        for (i = 0; i < s->nr_slots; ++i) {
            ...
            if (dtb.guest_phys_addr >= s->slots[i].start && dtb.guest_phys_addr < slot_end) {
                ...
                " lend=%d (offset=0x%"
                PRIx64
                ")", i, s->slots[i].start, s->slots[i].size, s->slots[i].lend, (uint64_t)(
                        dtb.guest_phys_addr - s->slots[i].start));
            }
        }
        for (i = 0; i < s->nr_slots; ++i) {
            if (s->slots[i].size == 0) continue;
